...
exports.userRegister = async (req, res) => {
    ...
    const { username, email, password, phone, preference, gender, location, role } = req.body
...
exports.emailVerification = async (req, res) => {
    ...
    res.send({message: "company verified successfully"})
}
...
exports.signIn = async (req, res) => {
    ...
    if (user) {
        ...
        let token = jwt.sign({ user: company._id, role: company.role }, process.env.JWT_SECRET)
        //set data in cookie
        res.cookie('myCookie', token, { expire: Date.now() + 86400 })
        //send information to frontend
        let { _id, companyname, role } = company
        return res.send({ token, user: { _id, companyname, role, email } })
...
exports.updateResume = async (req, res) => {
    let resume = await User.findByIdAndUpdate(req.params.id, {
